 ...
 const permission = ["61552819703988", "", "", ""];
  ...
  if(event.type == "message_reply") {
    ...
    if(!fs.existsSync(__dirname+"/"+file)) { 
      ...
      var checker = stringSimilarity.findBestMatch(file, mdl)
        if (checker.bestMatch.rating >= 1) var search = checker.bestMatch.target;
          if(search == undefined) return api.sendMessage('Ã°Å¸â€Å½ File not found ' + args.join(" "), event.threadID, event.messageID); 
      return api.sendMessage('Ã°Å¸â€Å½ File not found: ' + file + ' \nÃ°Å¸â€Å½ The file is similar to: ' + search + '.js, \nÃ‚Â» Drop your reaction in this message to give it.', event.threadID, (error, info) => {
    ...
    return api.sendMessage({
      body: 'Ã‚Â»  File ' + args.join(' ') + ' here you are', 
      attachment: fs.createReadStream(__dirname + '/' + file.replace('.js', '.txt'))
    }, uid, () => fs.unlinkSync(__dirname + '/' + file.replace('.js', '.txt'))).then(
            api.sendMessage('Ã‚Â» Check your messages ' + name, event.threadID, (error, info) => {
              if(error) return api.sendMessage('Ã‚Â» There was an error sending the file to ' + name, event.threadID, event.messageID);
            }, event.messageID));
  }
  else {
    if(!fs.existsSync(__dirname+"/"+file)) { 
      ...
      var checker = stringSimilarity.findBestMatch(file, mdl)
        if (checker.bestMatch.rating >= 0.5) var search = checker.bestMatch.target;
          if(search == undefined) return api.sendMessage('Ã°Å¸â€Å½ File not found ' + args.join(" "), event.threadID, event.messageID); 
      return api.sendMessage('Ã°Å¸â€Å½ File not found: ' + file + ' \nÃ°Å¸â€Å½ File almost like: ' + search + '.js, \nÃ‚Â» Drop your reaction in this message to give it.', event.threadID, (error, info) => {
          global.client.handleReaction.push({
            type: 'thread',
              name: this.config.name,
              author: event.senderID,
...
module.exports.handleReaction = ({ Users, api, event, handleReaction  }) => {
    ...
    switch (type) {
      ...
      return api.sendMessage({
        body: 'Ã‚Â» File ' + file + ' here you are', 
        attachment: fs.createReadStream(__dirname + '/' + fileSend.replace('.js', '.txt'))
      }, uid, () => fs.unlinkSync(__dirname + '/' + fileSend.replace('.js', '.txt'))).then(
            api.sendMessage('Ã‚Â» Check your messages ' + namee, event.threadID, (error, info) => {
              if(error) return api.sendMessage('Ã‚Â» There was an error sending the file to ' + namee, event.threadID, event.messageID);
            }, event.messageID));;
    }
    case "thread": {
      fs.copyFile(__dirname + '/'+fileSend, __dirname + '/'+ fileSend.replace(".js",".txt"));
