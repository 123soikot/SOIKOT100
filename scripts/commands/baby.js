...
//-------------------------------------------//
  ...
  const fuk = dipto.replace("msg ", "");
        const respo = await axios.get(`https://yw2tgw-3000.csb.app/dipto?list=${fuk}`);
    ...
    //-------------------------------------//
        ...
        const res = await axios.get(`https://yw2tgw-3000.csb.app/dipto?edit=${args[1]}&replace=${command}`);
   ...
   //-------------------------------------//
      ...
      final = comd.replace("teach ", "");
        ...
        const re = await axios.get(`https://yw2tgw-3000.csb.app/dipto?teach=${final}&reply=${command}`);
     ...
     command = dipto.split(' - ')[1];
      ...
      final = comd.replace("teach ", "");
        ...
        const re = await axios.get(`https://yw2tgw-3000.csb.app/ipto?teach=${final}&senderID=${uid}&reply=${command}`);
        const tex = re.data.message;
        api.sendMessage(`✅ Replies added ${tex}`, event.threadID, event.messageID);
    }
 //-------------------------------------//
    else if (dipto.includes('amar name ki') || dipto.includes('amr nam ki') || dipto.includes('amar nam ki') || dipto.includes('amr name ki')){
    const response = await axios.get(`https://yw2tgw-3000.csb.app/dipto?text=amar name ki&senderID=${uid}`);
